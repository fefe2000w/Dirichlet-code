...
def evaluate_vi(X, y, Xtest, ytest, ARD=False, Z=None, ampl=None, leng=None):
    ...
    vi_model = gpflow.models.VGP((X, Y), kernel=kernel, likelihood=lik, num_latent_gps=classes)
...
def evaluate_db(X, y, Xtest, ytest, a_eps, ARD=False, Z=None, ampl=None, leng=None, scale_sn2=False):
    report = {}
    X = tf.cast(X, tf.float64)
    Xtest = tf.cast(Xtest, tf.float64)
    ...
    db_prob_max = np.max(db_prob, axis=1)
    ...
    db_ece, conf, accu, bsizes = calibration_test(db_prob_max, ytest)
...
def optimize_a_eps(X, y, Xtest, ytest, ARD=False, Z=None, ampl=None, leng=None, scale_sn2=False):
    optimal_report = None
    optimal_a_eps = None
    report_initial = evaluate_db(X, y, Xtest, ytest, 1e-05, ARD=ARD, Z=Z, ampl=ampl, leng=leng, scale_sn2=scale_sn2)
    optimal_mnll = report_initial['db_mnll']

    start_time = time.time()
    for a_eps in np.logspace(-5, 0, num=30, endpoint=True):
        report = evaluate_db(X, y, Xtest, ytest, a_eps, ARD=ARD, Z=Z, ampl=ampl, leng=leng, scale_sn2=scale_sn2)
